...
# Linea de muelle
REF_A = (35.887658, -5.500033)
REF_B = (35.892863, -5.493891)

# Offset XY de la linea de BERTH para 3xS
REF_OFFSET = (200, 120)
...
# Radios de la tierra [Km]
R_AVG = 6371008.8           # Radio medio
R_POL = 6356752.3142        # Radio polar
R_EQU = 6378137.0           # Radio meridiano

# R_TER = R_AVG
R_TER = 6357976.759         # Radio de la terminal según su Lat

# CTE proporción [m/grado]
XY2LL = 111.19508023352181     # distance_1( (0,0) , (0,1) )
LL2XY = 1 / XY2LL 

# Coordenadas Lat / Lon del punto ZERO
ZERO = (35.88710239992243, -5.5025350392891506)

# Inclinación del mapa de la terminal en grados, respecto al eje latitud
INCLINATION = 43.70902515464513

# Parámetro que define el sentido del eje Y
FLIP_Y = -1

# Desplazamiento lineal del Mik con el centro de posicionamiento, en metros
MIK_D = 20.3

# Desplazamiento angular del MIK con el centro de posicionamiento, en grados
MIK_A = -0.0000094

# Ángulos de aplicación del Decaler, por zonas
DIC_DECALER = {
                'BLO013' : 17.7,
                'BLO014' : 18.95,
                'BLO015' : 90
...
# Para almacenar los decaler, se ha creado un diccionario para los ángulos

### Desplazamiento
def decaler(point, ang, dis = MIK_D):
...
def expand_line(line, cant):
    ...
    # Ahora hay que hacer el polígono con Decaler
    p0 = decaler(line[0], ang - 90, cant)
    p1 = decaler(line[1], ang - 90, cant)
    p2 = decaler(line[1], ang + 90, cant)
    p3 = decaler(line[0], ang + 90, cant)
